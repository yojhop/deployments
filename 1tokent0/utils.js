...
function produceHash(project){
    return new Promise((resolve,reject)=>{
        readConfig(project).then(config=>{
            const filePath=`./${project}/build/${config.build.productName}-Setup-v${config.version}.exe`
...
function putFilesToOss({t0env,project}){
    ...
    client.useBucket('otimg')
        readConfig(project).then(config=>{
            const fileNames=[`${config.build.productName}-Setup-v${config.version}.exe`,`${config.build.productName}-Setup-v${config.version}.exe.blockmap`,'latest.yml']
            try{
                const promises=[]
                ...
                if(t0env==='product'){
                    promises.push(client.put(`${downLoadFolder}/${config.build.productName}_Setup_v${config.version}.exe`, `${sourceFolder}/${config.build.productName}-Setup-v${config.version}.exe`,{timeout:1800000}))
                }
...
function sign(project){
    return new Promise((resolve,reject)=>{
        readConfig(project).then(config=>{
            exec(`signtool sign /tr http://timestamp.digicert.com /td sha256 /fd sha256 /a ./${project}/build/${config.build.productName}-Setup-v${config.version}.exe"`,(error, stdout, stderr) => {
                if (error) {
                    reject(error)
                }
                else{
